package com.wallet.account.serviceImpl.password;
...
public class PasswordServiceImpl implements PasswordService {
    ...
    public void set(Long userId, String password, String oldPassword) {
        ...
        if (oldPassword == null || !encoder.matches(oldPassword, exist.getPasswordHash())) {
            throw new CommonException(ErrorCode.PASSWORD_WRONG, "旧密码不正确");
        }
    ...
    public String verifyAndIssue(Long userId, String password, String orderNo, long amount) {
        if (isLocked(userId)) {
            throw new CommonException(ErrorCode.PASSWORD_LOCKED, "userId=" + userId);
        }
        PayPassword passwordRow = store.findByUserId(userId);
        if (passwordRow == null || !"ENABLED".equals(passwordRow.getStatus())) {
            throw new CommonException(ErrorCode.PASSWORD_NOT_SET, "userId=" + userId);
        }
        if (!encoder.matches(password, passwordRow.getPasswordHash())) {
            recordFail(userId);
            throw new CommonException(ErrorCode.PASSWORD_WRONG, "userId=" + userId);
        }
        clearFail(userId);
    ...
    public void consumeTicket(String ticket, Long userId, String orderNo, long amount) {
        ...
        if (value == null) {
            throw new CommonException(ErrorCode.TICKET_INVALID, "票据不存在或已被使用");
        }
        ...
        if (!matches) {
            throw new CommonException(ErrorCode.TICKET_INVALID, "票据与本次支付不匹配");
        }
    }
