package com.wallet.pay.serviceImpl.channelConfig;
...
/**
 * 渠道配置服务，实现 {@link ChannelConfigService} 契约：商户密钥等配置落 channel_config 表，
 * 不写死在 yml。渠道实现（channel 模块）经契约接口取配置，不感知存储。
 *
...
public class ChannelConfigServiceImpl implements ChannelConfigService {
    ...
    public <T> T requireEnabled(String channelCode, Class<T> type) {
        ...
        try {
            ...
            throw new ChannelException(ErrorCode.CHANNEL_INVOKE_ERROR, "渠道配置格式错误: " + channelCode);
        }
    }
