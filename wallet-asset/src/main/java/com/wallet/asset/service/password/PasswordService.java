...
public class PasswordService {
    ...
    public PasswordService(PasswordStore store, RedissonClient redisson, PasswordProperties props) {
        this.store = store;
    ...
    public void set(Long userId, String password, String oldPassword) {
        ...
        if (oldPassword == null || !encoder.matches(oldPassword, exist.getPasswordHash())) {
            throw new BizException(AssetError.PASSWORD_WRONG, "旧密码不正确");
        }
    ...
    public String verifyAndIssue(Long userId, String password, String orderNo, long amount) {
        if (isLocked(userId)) {
            throw new BizException(AssetError.PASSWORD_LOCKED, "userId=" + userId);
        }
        PayPassword passwordRow = store.findByUserId(userId);
        if (passwordRow == null || !"ENABLED".equals(passwordRow.getStatus())) {
            throw new BizException(AssetError.PASSWORD_NOT_SET, "userId=" + userId);
        }
    ...
    public void consumeTicket(String ticket, Long userId, String orderNo, long amount) {
        ...
        if (value == null) {
            throw new BizException(AssetError.TICKET_INVALID, "票据不存在或已被使用");
        }
        ...
        if (!matches) {
            throw new BizException(AssetError.TICKET_INVALID, "票据与本次支付不匹配");
        }
    }
