...
/**
 * SDK 错误码。code 值即宿主的 i18n 资源键，宿主捕获 {@code PayChannelException}
 * 后按 {@code getErrorCode().getCode()} 渲染本地化文案。
 *
 * <p>与 CRMEB 宿主既有资源键同名的错误码（ORDER_HAS_PAID 等）可直接复用存量文案。</p>
 */
public enum PayErrorCode {
    ...
    /** 上一笔交易关闭失败，等待人工处理（保留 CRMEB 原资源键拼写） */
    ORDER_PAYING_WAITE_REFUND("ORDER_PAYING_WAITE_REFUND"),

    /** 非法状态流转（保留 CRMEB 原资源键） */
